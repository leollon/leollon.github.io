...
这里引入一些其他可以在TCPIP网络中常见的连接设备，比如交换机，集线器以及网桥。
...
Bluetooth 标准定义的事OSI 数据链路层以及物理层（等同于TCPIP的网络接入层）。Bluetooth商标由一个称为蓝牙特殊爱好组（Special Interest Group，SIG）来控制。蓝牙专注于为短距离（10米）范围内无线设备提供一个可靠并高性能的通信环境。
...
- 支持更上层协议系统的一族网络控制协议（NCPs）：PPP可以包含有给TCPIP和备用网络协议提供单独接口的单独子层。
...
**网桥**是通过物理地址过滤以及转发的连接设备。网桥在OSI的数据链路层工作（与TCPIP四层中网络接入层相对应）。尽管网桥不是一个路由器，网桥依然使用一个转发表作为传输信息的源头。这个以物理地址为基本的路由表与路由表使用使用的表十分不同并且更简单一些。
